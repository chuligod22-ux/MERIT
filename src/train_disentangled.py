# -*- coding: utf-8 -*-
"""SP03 B — train the physics-disentangled (dual-branch) restorer. Fair param budget (width 22 ~4.7M < single 4.95M)."""
import os, sys, time
...
CKPT = os.path.join(WEIGHTS, "restorer_disentangled.pt")
dev = "cuda"
...
net = DisentangledRestorer(width=22).to(dev)
...
for i in range(STEPS):
    ...
    for c in cl:
        ...
        bo = gaussian_filter(c, (svb, shb))                          # noiseless blur-only (intermediate target)
    ...
    l_den = l1f(den, bo_t)                                            # physical noise/optics supervision
    ...
    if i % 500 == 0 or i == STEPS - 1:
        ...
        acc = [0.0, 0.0, 0.0]
    if i > 0 and i % 1500 == 0:
        torch.save({"state": net.state_dict(), "width": 22}, CKPT)
