...
ckd = torch.load(os.path.join(WEIGHTS, "restorer_parfusion.pt"), map_location=dev)
disent = ParallelFusionRestorer(width=ckd["width"]).to(dev).eval(); disent.load_state_dict(ckd["state"])
u8 = lambda a: np.clip(a, 0, 255).astype(np.uint8)
print(f"MERIT(single) {sum(p.numel() for p in merit.parameters())/1e6:.2f}M  | "
      f"ParFusion    {sum(p.numel() for p in disent.parameters())/1e6:.2f}M\n")
...
print(f"  ParFusion      MTF50 {np.mean(md):.3f}+-{np.std(md):.3f} ({np.mean(md)/m_sharp*100:.0f}%)  PSNR {np.mean(pd):.1f}")
...
print(f"  ParFusion      {rd2.mean():+.0f}% +- {rd2.std():.0f}   | disent>single in {wins}/{len(rm)}")
